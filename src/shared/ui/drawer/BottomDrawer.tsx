...
type BottomDrawer = {
	...
	customPiska?: ReactNode;
} & DrawerProps;

export function BottomDrawer(props: BottomDrawer) {
	const {openDrawerRef, closeDrawerRef, content, customPiska, afterAutoCloseAction, closeDisabled} = props;
	...
	return (
		<Drawer.Root dismissible={!closeDisabled}>
			...
			<Drawer.Portal>
				...
				<Drawer.Content className='fixed bottom-0 left-0 right-0 flex max-h-[96%] flex-col rounded-t-[10px] bg-white'>
					<div className='mx-auto flex w-full max-w-md flex-col overflow-auto rounded-t-[10px] p-4'>
						{customPiska ? customPiska : <div className='mx-auto mb-6 h-1.5 w-12 rounded-full bg-zinc-300' />}
